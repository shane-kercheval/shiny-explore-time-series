...
predict_horizon <- 20
reg_formula <- regression_build_formula(dependent_variable = 'Loaded Data',
                                        independent_variables = independent_variables)

# dataset <- visnights
# independent_variables = c('trend', 'season', 'NSWNthCo')
# predict_horizon <- 20  # if we don't include any independent variables other than trend/season
# predict_horizon <- 4
# reg_formula <- regression_build_formula(dependent_variable = 'NSWMetro',
#                                         independent_variables = independent_variables)

ts_model <- tslm(formula = reg_formula, data = dataset)


prediction_type = 'predictions of predictions' # think of something better
...
if(all(independent_variables %in% c('trend', 'season'))) {
    ...
    if(prediction_type == 'predictions of predictions') {
        
        # FIGURE OUT LAG
        # the furthest we can predict is the number of lagged values we use
        
        # use all lagged values, make the next prediction, use that prediction to predict the next, and so on

        # get lagged dataset for last period in dataset
        
        ts_forecast <- NULL
        predictions <- c(4.886140)  # starting value e.g. lagged value
        for(horizon in 1:predict_horizon) {
            print(horizon)
            # last predition has all x predictions
            ts_forecast <- forecast(ts_model, newdata = data.frame(NSWNthCo=predictions))
            temp <- as.data.frame(ts_forecast)$`Point Forecast`
            predictions <- c(predictions, temp[length(temp)])
        }
        #ts_forecast
        #rownames(as.data.frame(ts_forecast)
    } else {
        
        # ensure we are using only lagged values greather than or equal to the horizon we are predicting
    }
}
